...
class _BufferedTeeStream:
    """
    Buffers output in memory and writes to original stream only.
    No log files are created until flush_to_error_log() is called.
    """
...
def attach_log_streams(module_name: str) -> None:
    ...
    Logs are kept in memory only - no log files are created.
    ...
    if not isinstance(sys.stdout, _BufferedTeeStream):
        _buffered_stdout = _BufferedTeeStream(sys.stdout)
        sys.stdout = _buffered_stdout

    if not isinstance(sys.stderr, _BufferedTeeStream):
        _buffered_stderr = _BufferedTeeStream(sys.stderr)
