...
def _load_font(font_size):
    ...
    for name in font_names:
        try:
            ...
            print(f"[Font] Loaded system font: {name} at {font_size}px")
            return font
        except (OSError, IOError):
            continue

    # Fallback: try default (will be small, but better than crashing)
    print(f"[Font] ⚠ WARNING: No system font found! Using default bitmap font")
    print(
        f"[Font] ⚠ Default font will be very small (~6-8px) regardless of font_size={font_size}px"
    )
    print(
        f"[Font] ⚠ Consider installing system fonts (Arial, Helvetica, etc.) for proper text rendering"
    )
...
def _make_textbox_clip(
    ...
    bbox = draw.multiline_textbbox(
        (0, 0), wrapped, font=font, align="center", spacing=spacing
    )
    ...
    txt_clip = (
        ImageClip(np.array(canvas).astype("uint8"))
            ...
            if i == 1:
                ...
                for idx, message in enumerate(texts):
                    ...
                    overlay = (
                        ...
                        .with_start(start_t)
                        .with_effects([vfx.FadeIn(txt_fade), vfx.FadeOut(txt_fade)])
                    )
...
def parse_args(argv):
    ...
    p.add_argument(
        ...
        choices=["crop", "pad"],
        default="crop",
        help="Square conversion: crop or pad",
    )
