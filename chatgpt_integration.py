...
# Canonical text overlay locations (aligned with merge.py)
LOCATION_NAMES = {
    "top-center",
    "top-left",
    "top-right",
...
"""
        ...
        *,
        ...
        try:
            ...
            prompt = self._build_prompt_template(
                category, subcategory, additional_context
            )
    ...
    def formatForVideo(
        self, script_response: Optional[Dict[str, Any]]
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for i, text_obj in enumerate(texts_array):
                ...
                if not content:
                    logger.warning(
                        "[GPT] Text object %d has empty content, skipping", i
                    )
                    continue
    ...
    def validateScript(
        self, formatted_script: Optional[Dict[str, Any]]
        ...
        if len(texts) != len(locations):
            return (
                False,
                f"Texts ({len(texts)}) and locations ({len(locations)}) arrays have different lengths",
            )
        ...
        for i, (text, location) in enumerate(zip(texts, locations)):
            ...
            if len(text) > 500:
                return (
                    False,
                    f"Text at index {i} is too long ({len(text)} chars, max 500)",
                )
...
def createVideoFromCategory(
    ...
    script = gpt.generateScript(
        category, subcategory, additional_context=additional_context
    )
    if not script:
...
def send_categories_to_chatgpt(
    ...
    """
    gpt = get_gpt_instance()
    return gpt.generateScript(
        category, subcategory, additional_context=additional_context
    )


def generate_three_text_segments(
    category: str,
    subcategory: str = "",
    *,
    additional_context: Optional[str] = None,
    ...
    """
    Convenience wrapper to ensure we always return exactly three text segments.

    The legacy createVideoFromCategory() path may return 1..N texts. This helper
    enforces three items by truncating extras or duplicating the last valid item.
    Locations are normalized to a single preferred value when missing.
    """
    gpt = get_gpt_instance()
    raw = gpt.generateScript(
        category=category,
        subcategory=subcategory,
        additional_context=additional_context,
    )
    formatted = gpt.formatForVideo(raw)
    if not formatted:
        logger.error("[generate_three_text_segments] No formatted output from GPT")
        return None

    texts = formatted.get("texts") or []
    locs = formatted.get("text_locations") or []

    # Normalize locations: fall back to a single default
    normalized_locs: List[str] = []
    for loc in locs[:3]:
        loc_str = (loc or default_location or "bottom").strip().lower()
        if loc_str not in LOCATION_NAMES:
            loc_str = default_location
        normalized_locs.append(loc_str)

    # Ensure we have three texts; duplicate the last valid one if needed
    if texts:
        while len(texts) < 3:
            texts.append(texts[-1])
    else:
        # Absolute fallback when GPT returns nothing usable
        texts = [
            f"{category or subcategory} — key benefit",
            "What problem it solves",
            "Call to action",
        ]
    texts = texts[:3]

    # Align locations length with texts (pad with default)
    while len(normalized_locs) < 3:
        normalized_locs.append(default_location or "bottom")
    normalized_locs = normalized_locs[:3]

    return {
        "texts": texts,
        "text_locations": normalized_locs,
